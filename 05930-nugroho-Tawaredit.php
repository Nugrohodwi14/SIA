...
<head>
    ...
    function kode_klp_klik() {
        ...
        $.ajax({
            type: 'post',
            url: '05930-nugroho-buatkdklp.php',
            data: {
                klp: klp
            },
...
<body onload="kode_klp_klik()">
	...
	$id		= $_GET['kode'];
	$sql = "select m.idmatkul as midm,d.npp as dnpp, k.klp as kklp, k.hari as khr, k.jamkul as kjkul, k.ruang as kruang, k.kapasitas as kkapasitas from matkul as m, dosen as d, kultawar as k
    ...
    <div class="utama">
        ...
        <form action="05930-nugroho-Tawarsv_edit.php" method="post">
            ...
            <div class="form-group">
                <label for="kapasitas">Kapasitas Ruang</label>
                <select class="form-control" name="kapasitas" id="kapasitas">
                    <?php
					$kapasitas=array('250','40','50','150','100','200');
					for($i=0;$i<count($kapasitas);$i++){
						if ($kapasitas[$i]==$row['kkapasitas']){
							echo "<option value=$kapasitas[$i] selected>$kapasitas[$i]";
						}else{
							echo "<option value=$kapasitas[$i]>$kapasitas[$i]";
						}
					}
					?>
                </select>
